...
export class CashierPaymentStateMachine {
    static assertTransition(from: CashierPaymentState, to: CashierPaymentState) {
        if (!transitions[from]?.includes(to)) {
        throw new Error(`INVALID_CASHIER_PAYMENT_TRANSITION: ${from} → ${to}`);
        }
    }
    
    static canTransition(from: CashierPaymentState, to: CashierPaymentState): boolean {
